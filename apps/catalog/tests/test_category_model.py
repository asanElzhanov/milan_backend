...
class CategoryModelTests(TestCase):
    def test_category_slug_is_generated_from_name_when_empty(self):
        category = Category.objects.create(name_ru='Winter Shoes')

        self.assertEqual(category.slug, 'winter-shoes')

    def test_category_active_queryset_returns_only_active_categories(self):
        active = Category.objects.create(name_ru='Active', slug='active')
        Category.objects.create(name_ru='Hidden', slug='hidden', is_active=False)

        self.assertEqual(list(Category.objects.active()), [active])

    def test_category_mptt_tree_fields_are_populated(self):
        root = Category.objects.create(name_ru='Shoes', slug='shoes')
        child = Category.objects.create(name_ru='Sneakers', slug='sneakers', parent=root)
