...
class ColorAPITests(APITestCase):
    ...
    def test_color_hex_code_accepts_valid_hex_value(self):
        color = Color(name_ru='Blue', slug='blue', hex_code='#3366FF')

        color.full_clean()

    def test_color_hex_code_rejects_invalid_hex_value(self):
        color = Color(name_ru='Broken', slug='broken', hex_code='3366FF')

        with self.assertRaises(ValidationError):
            color.full_clean()

    def test_color_slug_is_generated_from_name_when_empty(self):
        color = Color.objects.create(name_ru='Light Blue', hex_code='#ADD8E6')
